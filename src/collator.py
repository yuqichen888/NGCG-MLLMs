...
class TrainTextImageDataCollator:
    ...
    def __call__(self, examples):
        ...
        """
        # cur["qry"], cur["qry_image_path"], cur["pos_text"], cur["pos_image_path"]
        ...
        qry_inputs = self._get_batch_inputs(examples, "qry_text", "qry_img_path")
        pos_inputs = self._get_batch_inputs(examples, "pos_text", "pos_img_path")
        return qry_inputs, pos_inputs

    def _get_batch_inputs(self, examples, text_keyname, image_keyname):

        texts, images = [], []
        if len(examples) == len(examples[0]):
            examples = examples[0]
        for example in examples:
            ...
            if example is None or not example:
                text, image = '  ', None
            elif type(example) is dict:  # JIT-friendly check
                # Extract values using the provided keys
                text = example.get(text_keyname, '  ')
                image = example.get(image_keyname, None)
            else:
                text, image = example[text_keyname], example[image_keyname]

            if type(text) == list:
                if text is not None and len(text) > 0 and image is None:
                    text, image = text[0], None
                elif text is None and image is not None:
                    text,image = '  ', image[0]
                elif len(text) == 0 or len(image) == 0:
                    text, image = '  ', None
                # else:
        ...
        # inputs = process_vlm_inputs(inputs, processor=self.processor,backbone_name= self.model_args.model_backbone, max_length=self.data_args.max_len)
        return inputs
