...
def exponential_decay(last_lr, start_frame, end_frame, position, scheduler):
    # 计算衰减比例
    start_step = start_frame["position"]
    end_step = end_frame["position"]
    progress = (position - start_step) / (end_step - start_step)

    # 指数衰减公式（线性到对数空间）
    lr_start = start_frame["lr"]
    lr_end = end_frame["lr"]
...
def get_exponential_schedule_with_warmup(optimizer, lr_max, lr_end, num_warmup_steps, num_training_steps, last_epoch=-1):
    scheduler = KeyframeLR(
        ...
        frames=[
            ...
            {"transition": exponential_decay},  # ✅ 替换 lambda
        ],
